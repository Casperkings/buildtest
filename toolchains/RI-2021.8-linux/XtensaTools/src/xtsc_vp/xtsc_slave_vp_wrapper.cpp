...
namespace xtsc_slave_vp_xtsc_vp__xtsc_slave_vp_FastBuild {

using namespace conf;
using namespace std;

template<int DATA_WIDTH>
class xtsc_vp__xtsc_slave_vp0Creator : public ScObjectCreatorBase
{
public:
  ...
  sc_object* create ( const string& name ) {
    ...
    if (scml_property_registry::inst().hasProperty(scml_property_registry::MODULE, scml_property_registry::BOOL, hierach_name, "runtime_disabled") && 
      ...
      if (creationVerboseMode() >= 6) { std::cout << "xtsc_slave_vp/xtsc_vp::xtsc_slave_vp: STUB for " << hierach_name << " created." << std::endl; }
      conf::stub *result = new conf::stub(n);
       ...
       conf::stub_port_registrator<>::register_stub_port(&xtsc_vp::xtsc_slave_vp<DATA_WIDTH>::m_respond_port, "m_respond_port" , string(static_cast<sc_object*>(result)->name()) + ".m_respond_port" );
      return result;
    } else {
      if (creationVerboseMode() >= 6) { std::cout << "xtsc_slave_vp/xtsc_vp::xtsc_slave_vp: " << hierach_name << " created." << std::endl; }
      xtsc_vp::xtsc_slave_vp<DATA_WIDTH>* result = new xtsc_vp::xtsc_slave_vp<DATA_WIDTH>(name.c_str());
...
struct DllAdapter {
  DllAdapter() {
    ...
    ScObjectFactory::inst().addCreator ("xtsc_vp::xtsc_slave_vp0<128>", new xtsc_vp__xtsc_slave_vp0Creator<128>());
    ScObjectFactory::inst().addCreator ("xtsc_vp::xtsc_slave_vp0<256>", new xtsc_vp__xtsc_slave_vp0Creator<256>());
    ScObjectFactory::inst().addCreator ("xtsc_vp::xtsc_slave_vp0<32>", new xtsc_vp__xtsc_slave_vp0Creator<32>());
    ScObjectFactory::inst().addCreator ("xtsc_vp::xtsc_slave_vp0<512>", new xtsc_vp__xtsc_slave_vp0Creator<512>());
    ScObjectFactory::inst().addCreator ("xtsc_vp::xtsc_slave_vp0<64>", new xtsc_vp__xtsc_slave_vp0Creator<64>());
    ...
    if (::getenv("SNPS_SLS_DYNAMIC_LOADER_VERBOSE")) { std::cout << "xtsc_slave_vp/xtsc_vp::xtsc_slave_vp loaded." << std::endl; }
  }
  ~DllAdapter() {
  }
