...
namespace xtsc_dma_engine_vp_xtsc_vp__xtsc_dma_engine_vp_FastBuild {

using namespace conf;
using namespace std;

template<int DATA_WIDTH>
class xtsc_vp__xtsc_dma_engine_vp0Creator : public ScObjectCreatorBase
{
public:
  ...
  sc_object* create ( const string& name ) {
    ...
    if (scml_property_registry::inst().hasProperty(scml_property_registry::MODULE, scml_property_registry::BOOL, hierach_name, "runtime_disabled") && 
      ...
      if (creationVerboseMode() >= 6) { std::cout << "xtsc_dma_engine_vp/xtsc_vp::xtsc_dma_engine_vp: STUB for " << hierach_name << " created." << std::endl; }
      conf::stub *result = new conf::stub(n);
       ...
       conf::stub_port_registrator<>::register_stub_port(&xtsc_vp::xtsc_dma_engine_vp<DATA_WIDTH>::m_respond_port, "m_respond_port" , string(static_cast<sc_object*>(result)->name()) + ".m_respond_port" );
       conf::stub_port_registrator<>::register_stub_port(&xtsc_vp::xtsc_dma_engine_vp<DATA_WIDTH>::m_request_port, "m_request_port" , string(static_cast<sc_object*>(result)->name()) + ".m_request_port" );
      ...
      if (creationVerboseMode() >= 6) { std::cout << "xtsc_dma_engine_vp/xtsc_vp::xtsc_dma_engine_vp: " << hierach_name << " created." << std::endl; }
      xtsc_vp::xtsc_dma_engine_vp<DATA_WIDTH>* result = new xtsc_vp::xtsc_dma_engine_vp<DATA_WIDTH>(name.c_str());
...
struct DllAdapter {
  DllAdapter() {
    ...
    ScObjectFactory::inst().addCreator ("xtsc_vp::xtsc_dma_engine_vp0<128>", new xtsc_vp__xtsc_dma_engine_vp0Creator<128>());
    ScObjectFactory::inst().addCreator ("xtsc_vp::xtsc_dma_engine_vp0<32>", new xtsc_vp__xtsc_dma_engine_vp0Creator<32>());
    ScObjectFactory::inst().addCreator ("xtsc_vp::xtsc_dma_engine_vp0<64>", new xtsc_vp__xtsc_dma_engine_vp0Creator<64>());
    ...
    if (::getenv("SNPS_SLS_DYNAMIC_LOADER_VERBOSE")) { std::cout << "xtsc_dma_engine_vp/xtsc_vp::xtsc_dma_engine_vp loaded." << std::endl; }
  }
  ~DllAdapter() {
  }
