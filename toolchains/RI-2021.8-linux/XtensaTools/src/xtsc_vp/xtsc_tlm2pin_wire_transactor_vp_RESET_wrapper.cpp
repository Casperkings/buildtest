...
#include "xtsc_vp/xtsc_tlm2pin_wire_transactor_vp_RESET.h"
#include "cassert"
...
namespace xtsc_tlm2pin_wire_transactor_vp_RESET_xtsc_vp__xtsc_tlm2pin_wire_transactor_vp_RESET_FastBuild {

using namespace conf;
using namespace std;


class xtsc_vp__xtsc_tlm2pin_wire_transactor_vp_RESET0Creator : public ScObjectCreatorBase
{
public:
  ...
  sc_object* create ( const string& name ) {
    ...
    if (scml_property_registry::inst().hasProperty(scml_property_registry::MODULE, scml_property_registry::BOOL, hierach_name, "runtime_disabled") && 
      ...
      if (creationVerboseMode() >= 6) { std::cout << "xtsc_tlm2pin_wire_transactor_vp_RESET/xtsc_vp::xtsc_tlm2pin_wire_transactor_vp_RESET: STUB for " << hierach_name << " created." << std::endl; }
      conf::stub *result = new conf::stub(n);
       ...
       conf::stub_port_registrator<>::register_stub_port(&xtsc_vp::xtsc_tlm2pin_wire_transactor_vp_RESET::m_sc_out, "m_sc_out" , string(static_cast<sc_object*>(result)->name()) + ".m_sc_out" );
      ...
      if (creationVerboseMode() >= 6) { std::cout << "xtsc_tlm2pin_wire_transactor_vp_RESET/xtsc_vp::xtsc_tlm2pin_wire_transactor_vp_RESET: " << hierach_name << " created." << std::endl; }
      xtsc_vp::xtsc_tlm2pin_wire_transactor_vp_RESET* result = new xtsc_vp::xtsc_tlm2pin_wire_transactor_vp_RESET(name.c_str());
...
struct DllAdapter {
  DllAdapter() {
    ...
    ScObjectFactory::inst().addCreator ("xtsc_vp::xtsc_tlm2pin_wire_transactor_vp_RESET0", new xtsc_vp__xtsc_tlm2pin_wire_transactor_vp_RESET0Creator());
    ...
    if (::getenv("SNPS_SLS_DYNAMIC_LOADER_VERBOSE")) { std::cout << "xtsc_tlm2pin_wire_transactor_vp_RESET/xtsc_vp::xtsc_tlm2pin_wire_transactor_vp_RESET loaded." << std::endl; }
  }
  ~DllAdapter() {
  }
