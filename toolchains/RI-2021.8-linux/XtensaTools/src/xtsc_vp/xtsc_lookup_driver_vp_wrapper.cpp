...
namespace xtsc_lookup_driver_vp_xtsc_vp__xtsc_lookup_driver_vp_FastBuild {

using namespace conf;
using namespace std;

template<int ADDR_WIDTH, int DATA_WIDTH>
class xtsc_vp__xtsc_lookup_driver_vp0Creator : public ScObjectCreatorBase
{
public:
  ...
  sc_object* create ( const string& name ) {
    ...
    if (scml_property_registry::inst().hasProperty(scml_property_registry::MODULE, scml_property_registry::BOOL, hierach_name, "runtime_disabled") && 
      ...
      if (creationVerboseMode() >= 6) { std::cout << "xtsc_lookup_driver_vp/xtsc_vp::xtsc_lookup_driver_vp: STUB for " << hierach_name << " created." << std::endl; }
      conf::stub *result = new conf::stub(n);
       conf::stub_port_registrator<>::register_stub_port(&xtsc_vp::xtsc_lookup_driver_vp<ADDR_WIDTH, DATA_WIDTH>::m_lookup, "m_lookup" , string(static_cast<sc_object*>(result)->name()) + ".m_lookup" );
      return result;
    } else {
      if (creationVerboseMode() >= 6) { std::cout << "xtsc_lookup_driver_vp/xtsc_vp::xtsc_lookup_driver_vp: " << hierach_name << " created." << std::endl; }
      xtsc_vp::xtsc_lookup_driver_vp<ADDR_WIDTH, DATA_WIDTH>* result = new xtsc_vp::xtsc_lookup_driver_vp<ADDR_WIDTH, DATA_WIDTH>(name.c_str());
       cwr_sc_object_registry::inst().addPort(&result->m_lookup, string(static_cast<sc_object*>(result)->name()) + ".m_lookup" );
      return result;
    }
  }
};
...
struct DllAdapter {
  DllAdapter() {
    ...
    ScObjectFactory::inst().addCreator ("xtsc_vp::xtsc_lookup_driver_vp0<8, 32>", new xtsc_vp__xtsc_lookup_driver_vp0Creator<8, 32>());
    ...
    if (::getenv("SNPS_SLS_DYNAMIC_LOADER_VERBOSE")) { std::cout << "xtsc_lookup_driver_vp/xtsc_vp::xtsc_lookup_driver_vp loaded." << std::endl; }
  }
  ~DllAdapter() {
  }
