...
#include "xtsc_vp/xtsc_pin2tlm_wire_transactor_vp_CLOCK.h"
#include "cassert"
...
namespace xtsc_pin2tlm_wire_transactor_vp_CLOCK_xtsc_vp__xtsc_pin2tlm_wire_transactor_vp_CLOCK_FastBuild {

using namespace conf;
using namespace std;


class xtsc_vp__xtsc_pin2tlm_wire_transactor_vp_CLOCK0Creator : public ScObjectCreatorBase
{
public:
  ...
  sc_object* create ( const string& name ) {
    ...
    if (scml_property_registry::inst().hasProperty(scml_property_registry::MODULE, scml_property_registry::BOOL, hierach_name, "runtime_disabled") && 
      ...
      if (creationVerboseMode() >= 6) { std::cout << "xtsc_pin2tlm_wire_transactor_vp_CLOCK/xtsc_vp::xtsc_pin2tlm_wire_transactor_vp_CLOCK: STUB for " << hierach_name << " created." << std::endl; }
      conf::stub *result = new conf::stub(n);
       conf::stub_port_registrator<>::register_stub_port(&xtsc_vp::xtsc_pin2tlm_wire_transactor_vp_CLOCK::m_sc_port, "m_sc_port" , string(static_cast<sc_object*>(result)->name()) + ".m_sc_port" );
       conf::stub_port_registrator<>::register_stub_port(&xtsc_vp::xtsc_pin2tlm_wire_transactor_vp_CLOCK::m_sc_in, "m_sc_in" , string(static_cast<sc_object*>(result)->name()) + ".m_sc_in" );
      return result;
    } else {
      if (creationVerboseMode() >= 6) { std::cout << "xtsc_pin2tlm_wire_transactor_vp_CLOCK/xtsc_vp::xtsc_pin2tlm_wire_transactor_vp_CLOCK: " << hierach_name << " created." << std::endl; }
      xtsc_vp::xtsc_pin2tlm_wire_transactor_vp_CLOCK* result = new xtsc_vp::xtsc_pin2tlm_wire_transactor_vp_CLOCK(name.c_str());
...
struct DllAdapter {
  DllAdapter() {
    ...
    ScObjectFactory::inst().addCreator ("xtsc_vp::xtsc_pin2tlm_wire_transactor_vp_CLOCK0", new xtsc_vp__xtsc_pin2tlm_wire_transactor_vp_CLOCK0Creator());
    ...
    if (::getenv("SNPS_SLS_DYNAMIC_LOADER_VERBOSE")) { std::cout << "xtsc_pin2tlm_wire_transactor_vp_CLOCK/xtsc_vp::xtsc_pin2tlm_wire_transactor_vp_CLOCK loaded." << std::endl; }
  }
  ~DllAdapter() {
  }
