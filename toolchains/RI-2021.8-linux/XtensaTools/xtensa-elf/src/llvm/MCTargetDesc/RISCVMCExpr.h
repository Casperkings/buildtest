...
private:
  ...
  explicit RISCVMCExpr(const MCExpr *Expr, VariantKind Kind)
      : Expr(Expr), Kind(Kind), Comment() {}
#if defined(TENSILICA) || 1
  explicit RISCVMCExpr(const MCExpr *Expr, VariantKind Kind, StringRef S)
      : Expr(Expr), Kind(Kind), Comment(S) {}
#endif
...
#if defined(TENSILICA) || 1
  static const RISCVMCExpr *create(const MCExpr *Expr, VariantKind Kind,
                                   StringRef Comment, MCContext &Ctx);
#endif
...
#if defined(TENSILICA) || 1
  /// getString - Get Comment for kind VK_RISCV_Comment
  const StringRef getComment() const {
    if (getKind() == VK_RISCV_Comment)
      return Comment;
    return StringRef();
  }
#endif
