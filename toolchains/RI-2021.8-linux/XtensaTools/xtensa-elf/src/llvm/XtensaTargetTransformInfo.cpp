//===-- XtensaTargetTransformInfo.cpp - Xtensa specific TTI pass ---------===//
//
...
/// Xtensa target machine. It uses the target's detailed information to provide
...
bool XtensaTTIImpl::areInlineCompatible(const Function *Caller,
...
unsigned XtensaTTIImpl::getRegisterBitWidth(bool Vector) const {
  if (!Vector)
    return 32;
...
unsigned XtensaTTIImpl::getMaxInterleaveFactor(unsigned VF) const { return 1; }

bool XtensaTTIImpl::lowerInterleavedLoadValues(
...
bool XtensaTTIImpl::lowerInterleavedStoreValues(
    SmallVectorImpl<Value *> &InputVectors,
    SmallVectorImpl<Value *> &OutputVectors, unsigned Factor,
    IRBuilder<> &Builder) {
  Tensilica::TensilicaInterleavedAccess IA;
...
bool XtensaTTIImpl::isLegalInterleavedAccessType(unsigned Opcode, Type *VecTy,
                                                 unsigned Factor,
                                                 bool NeedsSExt) {
...
int XtensaTTIImpl::getInterleavedMemoryOpCostWithSignedness(
...
bool XtensaTTIImpl::getTgtMemIntrinsic(IntrinsicInst *Inst,
...
bool XtensaTTIImpl::isBBNotForHWLoop(BasicBlock *BB,
...
// Unrolling be taken care of by software pipelining phase
void XtensaTTIImpl::getUnrollingPreferences(
  ...
  // This may be prolematic, mostly want to be a bit similar to xcc, which
...
unsigned XtensaTTIImpl::getLoadStoreVecRegBitWidth(unsigned AS) const {
  return ST->loadStoreWidth() * 8;
}

unsigned XtensaTTIImpl::getNumberOfRegisters(bool Vector) const {
  if (!Vector)
    return 16;
...
int XtensaTTIImpl::getVectorInstrCost(unsigned Opcode, Type *Val,
...
bool XtensaTTIImpl::isLegalArithmeticInstr(
...
int XtensaTTIImpl::getArithmeticInstrCost(
    unsigned Opcode, Type *Ty, TTI::OperandValueKind Opd1Info,
    TTI::OperandValueKind Opd2Info, TTI::OperandValueProperties Opd1PropInfo,
    TTI::OperandValueProperties Opd2PropInfo, ArrayRef<const Value *> Args,
...
int XtensaTTIImpl::getShuffleCost(TTI::ShuffleKind Kind, Type *Ty, int Index,
                                  Type *SubTp) {
...
int XtensaTTIImpl::getCmpSelInstrCost(unsigned Opcode, Type *ValTy,
  ...
  if (ISD == ISD::SELECT) {
    ...
    if (NewCondVT != CondVT) {
      ...
      const auto *Entry = CostTableLookup(
          InstCostTbl, XtensaISD::EXTRACT_SUBVECTOR_LO_HALF, NewCondMVT);
      if (!Entry)
        return IllegalInstrCost;

      CondCost += Entry->Cost;

      Entry = CostTableLookup(InstCostTbl, XtensaISD::EXTRACT_SUBVECTOR_UP_HALF,
                              NewCondMVT);
      if (!Entry)
        return IllegalInstrCost;
  ...
  if (NewCondVT != CondVT) {
    ...
    const auto *Entry =
        CostTableLookup(InstCostTbl, XtensaISD::CONCAT_HALF_VECTORS, CondMVT);
    if (!Entry)
      return IllegalInstrCost;
    CondCost += Entry->Cost;
  }
...
bool XtensaTTIImpl::isLegalCastInstr(unsigned Opcode, Type *Dst, Type *Src,
                                     const Instruction *I) {
  return getCastInstrCost(Opcode, Dst, Src, I) < IllegalInstrCost;
}

bool XtensaTTIImpl::isLegalCmpSelInstr(unsigned Opcode, Type *ValTy,
                                       Type *CondTy, const Instruction *I) {
  return getCmpSelInstrCost(Opcode, ValTy, CondTy, I) < IllegalInstrCost;
}

unsigned XtensaTTIImpl::getScalarizationOverhead(Type *Ty, bool Insert,
                                                 bool Extract) {
...
TargetSIMDInfo *XtensaTTIImpl::getTargetSIMDInfo() {
  return TensilicaTargetSIMDInfo::getInstance(
      Ctx, TLI, ST->getXtensaConfigInfo().SIMDInfo);
}

int XtensaTTIImpl::getMemoryOpCost(unsigned Opcode, Type *Src,
...
bool XtensaTTIImpl::isLegalMaskedLoad(Type *DataTy, MaybeAlign Alignment) {
...
bool XtensaTTIImpl::isLegalMaskedStore(Type *DataTy, MaybeAlign Alignment) {
  static const MVT VecMVTs[] = {MVT::i32
...
bool XtensaTTIImpl::allowsVectorization(
...
const Type *XtensaTTIImpl::getTargetComplexFloatType() {
...
const Type *XtensaTTIImpl::getTargetComplexDoubleType() {
...
const TargetComplexOpInfo *XtensaTTIImpl::getTargetComplexFloatOpInfo() {
  static Tensilica::TensilicaComplexFloatInfo Info(ComplexFloatOpInfo);
  return &Info;
}
const TargetComplexOpInfo *XtensaTTIImpl::getTargetComplexDoubleOpInfo() {
  static Tensilica::TensilicaComplexFloatInfo Info(ComplexDoubleOpInfo);
  return &Info;
}

bool XtensaTTIImpl::isLegalMaskedGatherInst(CallInst *CI) {
...
bool XtensaTTIImpl::isLegalMaskedScatterInst(CallInst *CI) {
...
int XtensaTTIImpl::getIntrinsicInstrCost(Intrinsic::ID IID, Type *RetTy,
                                         ArrayRef<Type *> Tys,
...
int XtensaTTIImpl::getUintToFpInstrCost(unsigned Opcode, Type *Dst, Type *Src,
                                        const Instruction *I) {
...
int XtensaTTIImpl::getCastInstrCost(unsigned Opcode, Type *Dst, Type *Src,
                                    const Instruction *I) {
...
unsigned XtensaTTIImpl::getNumberOfParts(Type *Tp) {
  if (Tp->isVectorTy() &&
...
bool XtensaTTIImpl::isLSRCostLess(TargetTransformInfo::LSRCost &C1,
