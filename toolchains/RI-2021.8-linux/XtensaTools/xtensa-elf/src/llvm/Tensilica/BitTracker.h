...
#ifndef LLVM_LIB_CODEGEN_TENSILICA_BITTRACKER_H 
#define LLVM_LIB_CODEGEN_TENSILICA_BITTRACKER_H
...
struct BitTracker {
  ...
  typedef std::unordered_map<unsigned, RegisterCell> CellMapType;

#if defined(TENSILICA) || 1
  BitTracker(MachineEvaluator &E, MachineFunction &F);
#else
...
#if defined(TENSILICA) || 1
  MachineEvaluator &getME() { return ME; }
#endif
private:
#if defined(TENSILICA) || 1
  void visitPHI(const MachineInstr &PI, bool FromVisitedUse = false);
#else
...
#if defined(TENSILICA) || 1
public:
  static RegisterCell mergeRegCells(const RegisterCell &RC1,
                                    const RegisterCell &RC2,
                                    unsigned Reg);
  StringRef FunctionName() const { return MF.getName(); }
#endif
};

// Abstraction of a reference to bit at position Pos from a register Reg.
struct BitTracker::BitRef {
  BitRef(unsigned R = 0, uint16_t P = -1) : Reg(R), Pos(P) {}
...
struct BitTracker::RegisterRef {
  RegisterRef(unsigned R = 0, unsigned S = 0)
    : Reg(R), Sub(S) {}
  RegisterRef(llvm::Register R, unsigned S = 0)
    : Reg(R), Sub(S) {}
...
#if defined(TENSILICA) || 1
  RegisterCell(uint16_t Width = DefaultBitN) : Bits(Width), Done(true) {}
#else
...
#if defined(TENSILICA) || 1
  void setDone(bool IsDone) { Done = IsDone; }
  bool isDone() const { return Done; }
  bool isEqual(uint64_t x) const;
  bool isPow2OrNull(BitValue &V) const;
#endif
...
#if defined(TENSILICA) || 1
  bool Done; // is used for phi instuctions to mark that all argument has been
             // taken in account.
  void dump() const;
#endif
};
...
struct BitTracker::MachineEvaluator {
  MachineEvaluator(const RegisterInfo &T, MachineRegisterInfo &M)
      : TRI(T), MRI(M) {}
...
#if defined(TENSILICA) || 1
  static bool getConst(const BitTracker::RegisterCell &RC, uint16_t B, uint16_t W,
                uint64_t &U);
#endif
  // Helper functions.
  ...
  RegisterCell eINS(const RegisterCell &A1, const RegisterCell &A2,
                    uint16_t AtN) const;
#if defined(TENSILICA) || 1
  RegisterCell eCTZ(const RegisterCell &A1) const;
  RegisterCell eCLZ(const RegisterCell &A1) const;
  RegisterCell CTZ(const RegisterCell &A1) const;
  RegisterCell eCPOP(const RegisterCell &A1) const;
  RegisterCell eORC_B(const RegisterCell &A1) const;
  RegisterCell eREV8(const RegisterCell &A1) const;
  RegisterCell eROLI(const RegisterCell &A1, uint32_t N) const;
  RegisterCell eRORI(const RegisterCell &A1, uint32_t N) const;
  RegisterCell eROL(const RegisterCell &A1, const RegisterCell &A2) const;
  RegisterCell eROR(const RegisterCell &A1, const RegisterCell &A2) const;
  // Comparisons
  // Bit[0] is true if all bits specified by a mask M are set in A.
  RegisterCell eALL(const RegisterCell &A, const RegisterCell &M) const;

  // Bit[0] is true if any of the bits specified by a mask M are clear in A.
  RegisterCell eNALL(const RegisterCell &A, const RegisterCell &M) const;

  // Bit[0] is true if all of the bits specified by a mask M are clear in A.
  RegisterCell eNONE(const RegisterCell &A, const RegisterCell &M) const;

  // Bit[0] is true if any of the bits specified by a mask M are set in A.
  RegisterCell eANY(const RegisterCell &A, const RegisterCell &M) const;

  // Bit[0] is true if the bit A[N] is clear.
  RegisterCell eBC(const RegisterCell &A, const unsigned N) const;

  // Bit[0] is true if the bit A[N] is set.
  RegisterCell eBS(const RegisterCell &A, const unsigned N) const;

  // Bit[0] is true if A and B are equal.
  RegisterCell eEQ(const RegisterCell &A, const RegisterCell &B) const;

  // Bit[0] is true if A and B are not equal.
  RegisterCell eNE(const RegisterCell &A, const RegisterCell &B) const;

  // Bit[0] is true if signed A is not less than signed B.
  RegisterCell eGES(const RegisterCell &A, const RegisterCell &B) const;

  // Bit[0] is true if signed A is less than signed B.
  RegisterCell eLTS(const RegisterCell &A, const RegisterCell &B) const;

  // Bit[0] is true if signed A is not greater than signed B.
  RegisterCell eLES(const RegisterCell &A, const RegisterCell &B) const;

  // Bit[0] is true if signed A is greate than signed B.
  RegisterCell eGTS(const RegisterCell &A, const RegisterCell &B) const;

  // Bit[0] is true if unsigned A is not less than unsigned B.
  RegisterCell eGEU(const RegisterCell &A, const RegisterCell &B) const;

  // Bit[0] is true if unsigned A is less than unsigned B.
  RegisterCell eLTU(const RegisterCell &A, const RegisterCell &B) const;

  // Bit[0] is true if unsigned A is not greater than unsigned B.
  RegisterCell eLEU(const RegisterCell &A, const RegisterCell &B) const;

  // Bit[0] is true if unsigned A is greate than unsigned B.
  RegisterCell eGTU(const RegisterCell &A, const RegisterCell &B) const;
#endif
  ...
  virtual bool evaluate(const MachineInstr &BI, const CellMapType &Inputs,
                        BranchTargetList &Targets, bool &FallsThru) = 0;

  const RegisterInfo &TRI;
  MachineRegisterInfo &MRI;
#if defined(TENSILICA) || 1
  virtual void setBlockScanned(MachineBasicBlock &BB) = 0;
  virtual bool isBlockScanned(MachineBasicBlock &BB) const = 0;
  virtual void setBlockScanned(int BBN) = 0;
  virtual bool isBlockScanned(int BBN) const = 0;
  virtual void alwaysGoto(MachineBasicBlock *From,
                          MachineBasicBlock *To) = 0;
  virtual MachineBasicBlock *alwaysGoto(MachineBasicBlock *From) const = 0;
#endif
};
