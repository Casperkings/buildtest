...
class VarExtractor : public IRVisitor {
 public:
    using IRVisitor::visit;
    const Variable *var;
    VarExtractor() : var(nullptr) {}
...
private:
    ...
    Expr visit(const Call *op) override {
        if (op->call_type == Call::Halide ||
            ...
            std::stringstream outprinter;
            outprinter << op->args[0];
            std::string outname = outprinter.str();

            VarExtractor in;
            op->args[1].accept(&in);
            
            Parameter output_buf;
            auto it = env.find(outname);
            if (it != env.end()) {
              const Function &f = it->second.first;
              int idx = it->second.second;
            ...
            unsigned int n_dim = (op->args.size() - 2) / 3;
            
            std::vector<Expr> buffer_extents;
            
            if(output_buf.defined()) {
            
              for(int i = 0; i < output_buf.dimensions(); i++) {
              
                Expr temp;
                if( output_buf.extent_constraint(i).defined() && 
                    output_buf.min_constraint(i).defined()) {
                    
                  temp = output_buf.extent_constraint(i) - output_buf.min_constraint(i);
                  temp = mutate(simplify(mutate(temp)));
                }
                buffer_extents.push_back(temp);
              }
              
            } else if(in.var->param.defined()) {
            
              for(int i = 0; i < in.var->param.dimensions(); i++) {
              
                Expr temp;
                if( in.var->param.extent_constraint(i).defined() && 
                    in.var->param.min_constraint(i).defined()) {
                    
                  temp = in.var->param.extent_constraint(i) - in.var->param.min_constraint(i);
                  temp = mutate(simplify(mutate(temp)));
                }
                buffer_extents.push_back(temp);
              }
              ...
              // Push Buffer Extents
              // Specify Buffer extents for shared using : 
              //      <Func>.output_buffers()[0].dim().set_bounds().set_stride();
              
              auto output_buf = env.at(outname).first.output_buffers()[0];
              auto in_buf = env.at(in.var->name).first.output_buffers()[0];
              
              for(unsigned int i = 0; i < n_dim; i++) {
              
                Expr temp = output_buf.extent_constraint(i);
                ...
                } else if( in_buf.extent_constraint(i).defined() && 
                    in_buf.min_constraint(i).defined()) {
                    
                  temp = in_buf.extent_constraint(i) - in_buf.min_constraint(i);
                  temp = mutate(simplify(mutate(temp)));
                }
            ...
            std::vector<Expr> in_indexes, out_indexes, loop_extents;
            
            for(unsigned int i = 0, j = 2; i < n_dim; i++, j += 2) {
            
              in_indexes.push_back(op->args[j]);
              Expr temp = mutate(simplify(mutate(op->args[j+1] - op->args[j] + Expr(1))));
              loop_extents.push_back(temp);
            }
            
            for(unsigned int i = 0; i < n_dim; i++) {
              out_indexes.push_back(op->args[i + 2 + 2*n_dim]);
            }
            
            std::vector<Expr> call_extents;
            
            Expr prev_extent = Expr(1);
            int max_idx = 0;
            for(unsigned int i = 0; i < buffer_extents.size(); i++) {
              
              prev_extent *= loop_extents[i];
              
              if(!equal(buffer_extents[i], loop_extents[i])) {
              
                call_extents.push_back(prev_extent);
                prev_extent = Expr(1);
                
                if(call_extents.size() <= 2)
                  max_idx = i;
              }
            }
            
            // Add check for last dimensions flattened and fully flattened
            if(!equal(prev_extent, Expr(1))) {
              call_extents.push_back(prev_extent);
            }

            if(call_extents.size() == 1) {
              call_extents.push_back(Expr(1));
            }
            
            // If call_extents.size() > 2, then the user has to take care of unrolling 
            // the surrounding loops.
            
            Expr min_idx_in = mutate(flatten_args(in.var->name, in_indexes, in.var->image, in.var->param));
            Expr min_idx_out = mutate(flatten_args(outname, out_indexes, Buffer<>(), output_buf));

            // Make a new and improved DMA intrinsic

            // The cleanest way to get an accurate pointer is to generate a load and 
            // let it get mutated as needed.

            std::vector<Expr> new_args;

            Expr dummy_load_out = Load::make(op->type, outname, min_idx_out, Buffer<>(), output_buf, const_true(op->type.lanes()));
            new_args.push_back(dummy_load_out);

            Expr dummy_load_in = Load::make(op->type, in.var->name, min_idx_in, in.var->image, in.var->param, const_true(op->type.lanes()));
            new_args.push_back(dummy_load_in);

            new_args.push_back(call_extents[0]);
            new_args.push_back(call_extents[1]);

            // Output Row stride
            new_args.push_back(make_shape_var(outname, "stride", max_idx, Buffer<>(), output_buf));

            // Input Row stride
            new_args.push_back(make_shape_var(in.var->name, "stride", max_idx, in.var->image, in.var->param));

            // Constant 0
            new_args.push_back(Expr(0));
            
            return Call::make(op->type, "dma_copy", new_args, Call::Intrinsic);
