...
void CodeGen_GPU_Host<CodeGen_CPU>::compile_func(const LoweredFunc &f,
    ...
    for (pair<const DeviceAPI, CodeGen_GPU_Dev *> &i : cgdev) {
        ...
        if (target.has_feature(Target::CLCadence)) {
            // Dumps the kernel to the current directory for Cadence Xtensa to use
            char presentDir[4096];
            user_assert(getcwd(presentDir, sizeof(presentDir)));
            string filename = "/" + function_name + ".cl";
            filename = presentDir + filename;
            std::ofstream output_file(filename.c_str(), std::ios::out);
            std::copy(kernel_src.cbegin(), kernel_src.cend()-1,
                std::ostreambuf_iterator<char>(output_file));
            debug(2) << "Kernel written to " << filename << "\n";
        }
