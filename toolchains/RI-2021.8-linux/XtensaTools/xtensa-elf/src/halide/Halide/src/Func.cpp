        ...
        if (var_name_match(dims[i].var, var.name())) {
            ...
            if (!dims[i].is_pure() && var.is_rvar &&
                (t == ForType::Vectorized || t == ForType::Parallel ||
                 ...
                 t == ForType::GPULane || t == ForType::DMA)) {
...
Stage &Stage::dma_copy(VarOrRVar var) {
    set_dim_type(var, ForType::DMA);
    return *this;
}

Stage &Stage::mul_hi(VarOrRVar var) {
    set_dim_type(var, ForType::Parallel);
    return *this;
}
...
Func &Func::dma_copy(VarOrRVar var) {
    invalidate_cache();
    Stage(func, func.definition(), 0, args()).dma_copy(var);
    return *this;
}


Func &Func::mul_hi(VarOrRVar var) {
    invalidate_cache();
    Stage(func, func.definition(), 0, args()).mul_hi(var);
    return *this;
}
