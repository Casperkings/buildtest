...
void cornerHarris5x5_U8_ref(const uint8_t * const p_u8Src, uint8_t * const p_u8Dst,  const uint32_t u32Width, const uint32_t u32Height, const int32_t s32SrcStride, const int32_t s32DstStride, const uint32_t u32Shift);
...
void run_reference(uint8_t*      Src,
                  ...
                  int32_t        dstWidth,
                  int32_t        dstHeight) {
  cornerHarris5x5_U8_ref( ((uint8_t *)Src) + SrcPitch * IMAGE_PAD + IMAGE_PAD, Dst,  width, height, SrcPitch, DstPitch, 5);
}
...
#include "HarrisCornerU8.h"

int HarrisCornerU8_call(Buffer<uint8_t> in, Buffer<uint8_t> &out) {
  HarrisCornerU8(in, out);
  out.device_sync();
  out.copy_to_host();
  return 1;
}
...
int main(int argc, char **argv) {
  ...
  HarrisCornerU8_call(input, output);
