...
#include "TransposeU16.h"

int TransposeU16_call(Buffer<uint16_t> in, Buffer<uint16_t> &out) {
  TransposeU16(in, out);
  out.device_sync();
  out.copy_to_host();
  return 1;
...
int main(int argc, char **argv) {
  ...
  for (indy = IMAGE_PAD; indy < (srcHeight + IMAGE_PAD); indy++) {
    ...
    fread(ptr, 1, srcWidth*(SRC_BIT_DEPTH>>3), fpIn);
  }
  fclose(fpIn);
  ...
  for (int y = 0; y < input.height(); y++) {
    for (int x = 0; x < input.width(); x++)
        input(x, y) = ptr[y*input.height() + x];
  }

  Buffer<uint16_t> output(IMAGE_H, IMAGE_W);
  TransposeU16_call(input, output);
	ptr = (uint16_t*)gdst;
  for (int y = 0; y < output.height(); y++) {
    for (int x = 0; x < output.width(); x++) 
      ptr[y*output.height() + x] = output(x, y);
  }
  ...
  fwrite(gdst, IMAGE_W * IMAGE_H, DST_BIT_DEPTH>>3, fpOut);
  fclose(fpOut);
  
  printf("Compare: ");
  if (memcmp(gdst_ref, gdst, dstWidth*dstHeight*(DST_BIT_DEPTH>>3)) == 0) {
