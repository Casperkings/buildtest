...
#include "filter2D5x5.h"

Buffer<uint8_t> filter2D5x5_call(Buffer<uint8_t> in, Buffer<uint8_t> kernel)
{
  Buffer<uint8_t> out(in.width()-4, in.height()-4);
  filter2D5x5(in, kernel, out);
  out.device_sync();
  out.copy_to_host();
  return out;
}
...
int main(int argc, char **argv) {
  ...
  Buffer<uint8_t>kernel(5, 5);
  kernel(0, 0) = 6;  kernel(0, 1) = 17; kernel(0, 2) = 8;  
  kernel(0, 3) = 13; kernel(0, 4) = 2;
  kernel(1, 0) = 3;  kernel(1, 1) = 2;  kernel(1, 2) = 10; 
  kernel(1, 3) = 19; kernel(1, 4) = 15;
  kernel(2, 0) = 5;  kernel(2, 1) = 1;  kernel(2, 2) = 16; 
  kernel(2, 3) = 3;  kernel(2, 4) = 15;
  kernel(3, 0) = 18; kernel(3, 1) = 15; kernel(3, 2) = 6;  
  kernel(3, 3) = 13; kernel(3, 4) = 20;
  kernel(4, 0) = 4;  kernel(4, 1) = 16; kernel(4, 2) = 7;  
  kernel(4, 3) = 15; kernel(4, 4) = 7;

  Buffer<uint8_t> halide = filter2D5x5_call(input, kernel);
