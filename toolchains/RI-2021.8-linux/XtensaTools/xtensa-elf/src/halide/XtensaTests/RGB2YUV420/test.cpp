...
int rgb2yuv420_call(Buffer<uint8_t> in, Buffer<uint8_t> &out) {   
  rgb2yuv420(in, out);
  out.device_sync();
  out.copy_to_host();
  return 0;
...
int main()
{
  ...
  //Input buffer
  Buffer<uint8_t> input = Buffer<uint8_t>(3, IMAGE_W*2, IMAGE_H*2);
  ...
  rgb2yuv420_call(input, Out);

  for (int y = 0; y < IMAGE_H; y++) {
    for (int x = 0; x < IMAGE_W; x++) {
      gdstY[y*IMAGE_H + x] = Out(x, y, 0);
      gdstU[y/2*IMAGE_W/2 + x/2] = Out(x/2, y/2, 1);
      gdstV[y/2*IMAGE_W/2 + x/2] = Out(x/2, y/2, 2);
    }
  }
  ...
  fwrite(gdstY_ref, dstWidth * dstHeight, 1, fpOut);
  fclose(fpOut);
  
  // Write result
  sprintf(fname, "OutputU_ref.yuv");
  fpOut = fopen(fname, "wb");
  if (fpOut == NULL) {
    printf("Error opening output file: %s\n", fname);
    exit(0);
  ...
  fwrite(gdstU_ref, dstWidth * dstHeight / 4, 1, fpOut);
  fclose(fpOut);
  
  // Write result
  sprintf(fname, "OutputV_ref.yuv");
  fpOut = fopen(fname, "wb");
  if (fpOut == NULL) {
    printf("Error opening output file: %s\n", fname);
    exit(0);
  ...
  fwrite(gdstV_ref, dstWidth * dstHeight / 4, 1, fpOut);
  fclose(fpOut);
