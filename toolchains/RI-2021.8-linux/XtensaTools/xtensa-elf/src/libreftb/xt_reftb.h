...
// STL routines for HW simulation
extern int stl_start(int stat);
...
// Change STL info location
extern unsigned int* set_stl_info_location(unsigned int*);
...
static inline
int stl_info(int stat) 
{
  return stl_start(stat);
}
