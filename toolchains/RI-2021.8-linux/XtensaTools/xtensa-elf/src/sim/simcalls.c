...
{
  register int a2 __asm__ (CMD) = SYS_profile_enable;
...
{
  register int a2 __asm__ (CMD) = SYS_profile_disable;
  register int ret_val __asm__ (RETVAL);
  register int ret_err __asm__ (RETERR);
  __asm__ volatile (SIMCALL
		    : "=a" (ret_val), "=a" (ret_err)
...
{
  register int a2 __asm__ (CMD) = SYS_trace_level;
  register unsigned a3 __asm__ (ARG1) = level;
  register int ret_val __asm__ (RETVAL);
  register int ret_err __asm__ (RETERR);
  __asm__ volatile (SIMCALL
		    : "=a" (ret_val), "=a" (ret_err)
...
{
  register int a2 __asm__ (CMD) = SYS_client_command;
  register unsigned a3 __asm__ (ARG1) = (unsigned)client;
  register unsigned a4 __asm__ (ARG2) = (unsigned)command;
  register int ret_val __asm__ (RETVAL);
  register int ret_err __asm__ (RETERR);
  __asm__ volatile (SIMCALL
		    : "=a" (ret_val), "=a" (ret_err)
...
xt_iss_cycle_count()
{
  register unsigned int a2_in  __asm__ (CMD) = SYS_cycle_count;
  register unsigned int a2_out __asm__ (RETVAL);
  register unsigned int a3_out __asm__ (RETERR);
  __asm__ volatile (SIMCALL
		    : "=a" (a2_out), "=a" (a3_out)
		    : "a" (a2_in));
#if XCHAL_HAVE_BE
...
{
  register int a2  __asm__ (CMD) = SYS_peek;
  ...
  register unsigned int a3 __asm__ (ARG1) = (unsigned int) (address >> 32);
  register unsigned int a4 __asm__ (ARG2) = (unsigned int) address ;
  register unsigned int ret_val __asm__ (RETVAL);
  register int ret_err __asm__ (RETERR);
  __asm__ volatile (SIMCALL
		    : "=a" (ret_val), "=a" (ret_err)
		    : "a" (a2), "a" (a3), "a" (a4)); 
...
{
  register int a2  __asm__ (CMD) = SYS_poke;
  register unsigned int a3 __asm__ (ARG1) = src_value;
  /* No need to consider endianness when splitting a 64-bit address
     into two 32-bit registers because in iss4_simcall.cxx the same
     way will be used to join the 64-bit address back.  */
  register unsigned int a4 __asm__ (ARG2) = (unsigned int) (address >> 32);
  register unsigned int a5 __asm__ (ARG3) = (unsigned int) address ;
  register int ret_val __asm__ (RETVAL);
  register int ret_err __asm__ (RETERR);
  __asm__ volatile (SIMCALL
		    : "=a" (ret_val), "=a" (ret_err)
...
{
  register int a2 __asm__ (CMD) = SYS_addr_to_sym;
  register unsigned a3 __asm__ (ARG1) = (unsigned)addr;
  register unsigned a4 __asm__ (ARG2) = (unsigned)buf;
  register int ret_val __asm__ (RETVAL);
  register int ret_err __asm__ (RETERR);
  __asm__ volatile (SIMCALL
		    : "=a" (ret_val), "=a" (ret_err)
