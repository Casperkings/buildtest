...
void launch_ocl_TransposeU16(uint8_t *Src, uint32_t SrcPitch, uint8_t *Dst,
  ...
  cl_mem clSrc = clCreateBuffer(
      ...
      (sizeof(uint16_t) * (SrcPitch * (height + IMAGE_PAD + IMAGE_PAD))), NULL,
      &status);
  CHECK_ERROR(status);
  ...
  const char *progSource = loadKernel("TransposeU16.cl");
  cl_program program =
  ...
  local_src_size = ((TILE_W + IMAGE_PAD + IMAGE_PAD) *
                    (TILE_H + IMAGE_PAD + IMAGE_PAD) * sizeof(cl_ushort));
  ...
  error = clEnqueueWriteBuffer(
      ...
      (sizeof(uint16_t) * (SrcPitch * (height + IMAGE_PAD + IMAGE_PAD))), Src, 0,
      NULL, NULL);
  CHECK_ERROR(error);
  ...
  CHECK(clEnqueueReadBuffer(queue, clDst, CL_TRUE, 0,
                            (sizeof(uint16_t) * DstPitch * dstHeight), Dst, 0,
                            NULL, NULL));
...
void run_reference(uint8_t *Src, uint32_t SrcPitch, uint8_t *Dst,
  ...
  XI_TILE_SET_TYPE((&src_t), XI_TILE_U16);
...
int main() {
  ...
  launch_ocl_TransposeU16(gsrc, srcStride, gdst, dstWidth, srcWidth, srcHeight,
                      dstWidth, dstHeight);
