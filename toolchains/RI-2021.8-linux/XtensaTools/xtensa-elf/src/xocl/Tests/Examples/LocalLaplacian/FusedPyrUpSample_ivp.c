//#include "xi_lib.h"
{
  extern XI_ERR_TYPE xiPyrUp_S16S16(const xi_pTile src1, const xi_pTile src0,
                                    xi_pTile dst);
  ...
  int err = 0;
  err = xiPyrUp_S16S16(&src1_t, &src0_t, &dst_t);
  err = xiEdgeRepeatS16(&dst_t, (indY == 0), (indY == indY_size), (indX == 0),
                        (indX == indX_size));

  // err = xiPyrUp_S16S16_ref(&src1_t, &src0_t, &dst_t);
