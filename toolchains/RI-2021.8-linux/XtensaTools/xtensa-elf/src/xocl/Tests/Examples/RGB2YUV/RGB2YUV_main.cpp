...
void launch_ocl_kernel(uint8_t *Src, uint32_t SrcPitch, uint8_t *DstY,
  ...
  const char *progSource = loadKernel("RGB2YUV.cl");
  cl_program program =
  ...
  CHECK(clEnqueueReadBuffer(queue, clDstU, CL_TRUE, 0,
                            (sizeof(cl_char) * DstPitch * dstHeight), DstU, 0,
                            NULL, NULL));

  CHECK(clEnqueueReadBuffer(queue, clDstV, CL_TRUE, 0,
                            (sizeof(cl_char) * DstPitch * dstHeight), DstV, 0,
                            NULL, NULL));
...
void run_reference(uint8_t *Src, uint32_t SrcPitch, uint8_t *DstY,
  ...
  xiCvtColor_U8_RGB2YUV_BT709_ref((xi_pArray)&src_t, (xi_pArray)&dstY_t,
