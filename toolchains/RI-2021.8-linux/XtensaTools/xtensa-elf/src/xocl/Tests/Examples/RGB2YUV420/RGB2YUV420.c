...
#define Q15_0_3854 12629
#define Q15_0_1146 3755
#define Q15_0_4542 14883
#define Q15_0_0458 1501
...
#define Q15_0_1873 6137
#define Q15_0_4681 15339
#define Q15_0_7874 25802
#define Q15_0_9278 30402
#define Q15_0_0936 3068
#define Q15_0_2340 7669

#define Q15_1_8556 60804
#define Q15_1_5748 51603

#define XI_CLAMPNX16_0_255(v)                                                  \
  { v = IVP_MINNX16(255, IVP_MAXNX16(0, v)); }

XI_ERR_TYPE xiCvtColor_U8_RGB2YUV_420_BT709(const xi_pArray rgb, xi_pArray y,
                                            xi_pArray u, xi_pArray v) {
  ...
  xb_vec2Nx8U *restrict rsrc_rgb;
  xb_vec2Nx8U *restrict rdst_y;
  xb_vecNx8U *restrict rdst_u;
  xb_vecNx8U *restrict rdst_v;

  valign a_load;
  valign a_store = IVP_ZALIGN();

  // y = 0.2126f * r + 0.7152f * g + 0.0722f * b;
  // u = (b - y) * 0.5389f + 128.0f;
  // v = (r - y) * 0.6350f + 128.0f;

  int ix = 0;

#if XCHAL_HAVE_VISION
  for (; ix < width - 4 * XCHAL_IVPN_SIMD_WIDTH / 3;
       ix += 2 * XCHAL_IVPN_SIMD_WIDTH) {
    rsrc_rgb = OFFSET_PTR_2NX8U(psrc_rgb, 0, 0, 3 * ix);
    rdst_y = OFFSET_PTR_2NX8U(pdst_y, 0, 0, ix);
    rdst_u = OFFSET_PTR_NX8U(pdst_u, 0, 0, (ix >> 1));
    rdst_v = OFFSET_PTR_NX8U(pdst_v, 0, 0, (ix >> 1));

    int WX = width - XT_MAX(ix, width - 2 * XCHAL_IVPN_SIMD_WIDTH);

    int shift_rgb = XI_ARRAY_GET_PITCH(rgb) - (3 * WX);
    int shift_y = XI_ARRAY_GET_PITCH(y) - (WX);
    int shift_u = XI_ARRAY_GET_PITCH(u) - (WX >> 1);
    int shift_v = XI_ARRAY_GET_PITCH(v) - (WX >> 1);
    ...
    for (int iy = 0; iy < height; iy += 2) {
      ...
      IVP_L2A4NX8U_IP(vec1, vec0, a_load, rsrc_rgb);
#else
      IVP_LA2NX8U_IP(vec0, a_load, rsrc_rgb);
      IVP_LA2NX8U_IP(vec1, a_load, rsrc_rgb);
#endif
      ...
      xb_vec2Nx8U vec4;
#if XCHAL_VISION_TYPE == 6
      IVP_L2A4NX8U_IP(vec4, vec3, a_load, rsrc_rgb);
#else
      IVP_LA2NX8U_IP(vec3, a_load, rsrc_rgb);
      IVP_LA2NX8U_IP(vec4, a_load, rsrc_rgb);
#endif
      xb_vec2Nx8U vec5;
      IVP_LAV2NX8U_XP(vec5, a_load, rsrc_rgb,
                      3 * WX - 4 * XCHAL_IVPN_SIMD_WIDTH);
      rsrc_rgb = OFFSET_PTR_2NX8U(rsrc_rgb, 1, shift_rgb, 0);
      ...
      xb_vec2Nx8 vec_r1, vec_g1, vec_b1;
      IVP_DSEL2NX8I(vec_b1, tmp0, vec4, vec3,
                    IVP_DSELI_8B_DEINTERLEAVE_C3_STEP_0);
      IVP_DSEL2NX8I_H(vec_b1, tmp1, vec5, vec4,
                      IVP_DSELI_8B_DEINTERLEAVE_C3_STEP_1);
      IVP_DSEL2NX8I(vec_g1, vec_r1, tmp1, tmp0, IVP_DSELI_8B_DEINTERLEAVE_1);

#if XCHAL_VISION_TYPE == 6
      xb_vec2Nx24 vec_y0 =
          IVP_MULUSP2N8XR16(vec_g0, vec_r0, Q15_0_2126 | ((Q15_0_7152) << 16));
      IVP_MULUSAI2NX8X16(vec_y0, vec_b0, Q15_0_0722, Q15_0_0722);

      xb_vec2Nx24 vec_y1 =
          IVP_MULUSP2N8XR16(vec_g1, vec_r1, Q15_0_2126 | ((Q15_0_7152) << 16));
      IVP_MULUSAI2NX8X16(vec_y1, vec_b1, Q15_0_0722, Q15_0_0722);
#else
      xb_vec2Nx24 vec_y0 = IVP_MULUSI2NX8X16(vec_r0, Q15_0_2126, Q15_0_2126);
      IVP_MULUSAI2NX8X16(vec_y0, vec_g0, Q15_0_7152, Q15_0_7152);
      IVP_MULUSAI2NX8X16(vec_y0, vec_b0, Q15_0_0722, Q15_0_0722);

      xb_vec2Nx24 vec_y1 = IVP_MULUSI2NX8X16(vec_r1, Q15_0_2126, Q15_0_2126);
      IVP_MULUSAI2NX8X16(vec_y1, vec_g1, Q15_0_7152, Q15_0_7152);
      IVP_MULUSAI2NX8X16(vec_y1, vec_b1, Q15_0_0722, Q15_0_0722);
#endif
      ...
      xb_vec2Nx24 wb = IVP_ADDWU2NX8U(vec_b0, vec_b1);
      IVP_ADDWUS2NX8U(wb, vec_y_8u_0, vec_y_8u_1);

      xb_vec2Nx24 wr = IVP_ADDWU2NX8U(vec_r0, vec_r1);
      IVP_ADDWUS2NX8U(wr, vec_y_8u_0, vec_y_8u_1);

      xb_vecNx16U vec_b =
          IVP_ADDNX16(IVP_PACKL2NX24_0(wb), IVP_PACKL2NX24_1(wb));
      xb_vecNx16U vec_r =
          IVP_ADDNX16(IVP_PACKL2NX24_0(wr), IVP_PACKL2NX24_1(wr));

      xb_vecNx48 vec_u = IVP_MULNX16(vec_b, Q15_0_5389);
      xb_vecNx48 vec_v = IVP_MULNX16(vec_r, Q15_0_6350);

      xb_vecNx16 dst_u = IVP_ADDNX16(IVP_PACKVRNX48(vec_u, 17), 128);
      xb_vecNx16 dst_v = IVP_ADDNX16(IVP_PACKVRNX48(vec_v, 17), 128);

      IVP_SAV2NX8U_XP(vec_y_8u_0, a_store, rdst_y, WX);
      IVP_SAV2NX8UPOS_FP(a_store, rdst_y);
      rdst_y = OFFSET_PTR_2NX8U(rdst_y, 1, shift_y, 0);

      IVP_SAV2NX8U_XP(vec_y_8u_1, a_store, rdst_y, WX);
      IVP_SAV2NX8UPOS_FP(a_store, rdst_y);
      rdst_y = OFFSET_PTR_2NX8U(rdst_y, 1, shift_y, 0);

      IVP_SAVNX8U_XP(dst_u, a_store, rdst_u, WX >> 1);
      IVP_SAVNX8UPOS_FP(a_store, rdst_u);
      rdst_u = OFFSET_PTR_NX8U(rdst_u, 1, shift_u, 0);

      IVP_SAVNX8U_XP(dst_v, a_store, rdst_v, WX >> 1);
      IVP_SAVNX8UPOS_FP(a_store, rdst_v);
      rdst_v = OFFSET_PTR_NX8U(rdst_v, 1, shift_v, 0);
    }
  }

  if (ix < width) {
    xb_vec2Nx8U index_r = IVP_ADD2NX8U(
        IVP_ADD2NX8U(IVP_SEQ2NX8U(), IVP_SEQ2NX8U()), IVP_SEQ2NX8U());
    xb_vec2Nx8U index_g = IVP_ADD2NX8U(index_r, 1);
    xb_vec2Nx8U index_b = IVP_ADD2NX8U(index_r, 2);
    ...
    int shift_rgb = XI_ARRAY_GET_PITCH(rgb) - 3 * (width - ix);
    int shift_y = XI_ARRAY_GET_PITCH(y) - (width - ix);
    int shift_u = XI_ARRAY_GET_PITCH(u) - ((width - ix) >> 1);
    int shift_v = XI_ARRAY_GET_PITCH(v) - ((width - ix) >> 1);

    for (int iy = 0; iy < height; iy += 2) {
      a_load = IVP_LA2NX8U_PP(rsrc_rgb);
      xb_vec2Nx8U vec0;
      IVP_LAV2NX8U_XP(vec0, a_load, rsrc_rgb,
                      3 * (width - ix) - 0 * XCHAL_IVPN_SIMD_WIDTH);
      xb_vec2Nx8U vec1;
      IVP_LAV2NX8U_XP(vec1, a_load, rsrc_rgb,
                      3 * (width - ix) - 2 * XCHAL_IVPN_SIMD_WIDTH);
      rsrc_rgb = OFFSET_PTR_2NX8U(rsrc_rgb, 1, shift_rgb, 0);

      a_load = IVP_LA2NX8U_PP(rsrc_rgb);
      xb_vec2Nx8U vec3;
      IVP_LAV2NX8U_XP(vec3, a_load, rsrc_rgb,
                      3 * (width - ix) - 0 * XCHAL_IVPN_SIMD_WIDTH);
      xb_vec2Nx8U vec4;
      IVP_LAV2NX8U_XP(vec4, a_load, rsrc_rgb,
                      3 * (width - ix) - 2 * XCHAL_IVPN_SIMD_WIDTH);
      rsrc_rgb = OFFSET_PTR_2NX8U(rsrc_rgb, 1, shift_rgb, 0);

      xb_vec2Nx8 vec_r0, vec_g0, vec_b0;

      vec_r0 = IVP_SEL2NX8(vec1, vec0, index_r);
      vec_g0 = IVP_SEL2NX8(vec1, vec0, index_g);
      vec_b0 = IVP_SEL2NX8(vec1, vec0, index_b);

      xb_vec2Nx8 vec_r1, vec_g1, vec_b1;

      vec_r1 = IVP_SEL2NX8(vec4, vec3, index_r);
      vec_g1 = IVP_SEL2NX8(vec4, vec3, index_g);
      vec_b1 = IVP_SEL2NX8(vec4, vec3, index_b);
      ...
      xb_vec2Nx24 vec_y1 = IVP_MULUSI2NX8X16(vec_b1, Q15_0_0722, Q15_0_0722);
      IVP_MULUSAI2NX8X16(vec_y1, vec_g1, Q15_0_7152, Q15_0_7152);
      IVP_MULUSAI2NX8X16(vec_y1, vec_r1, Q15_0_2126, Q15_0_2126);
      ...
      IVP_SAV2NX8U_XP(vec_y_8u_0, a_store, rdst_y, (width - ix));
      IVP_SAV2NX8UPOS_FP(a_store, rdst_y);
      rdst_y = OFFSET_PTR_2NX8U(rdst_y, 1, shift_y, 0);

      IVP_SAV2NX8U_XP(vec_y_8u_1, a_store, rdst_y, (width - ix));
      IVP_SAV2NX8UPOS_FP(a_store, rdst_y);
      rdst_y = OFFSET_PTR_2NX8U(rdst_y, 1, shift_y, 0);

      IVP_SAVNX8U_XP(dst_u, a_store, rdst_u, (width - ix) >> 1);
      IVP_SAVNX8UPOS_FP(a_store, rdst_u);
      rdst_u = OFFSET_PTR_NX8U(rdst_u, 1, shift_u, 0);

      IVP_SAVNX8U_XP(dst_v, a_store, rdst_v, (width - ix) >> 1);
      IVP_SAVNX8UPOS_FP(a_store, rdst_v);
      rdst_v = OFFSET_PTR_NX8U(rdst_v, 1, shift_v, 0);
    }
  }
  ...
  for (; ix < width; ix += 2 * XCHAL_IVPN_SIMD_WIDTH) {
    rsrc_rgb = OFFSET_PTR_2NX8U(psrc_rgb, 0, 0, 3 * ix);
    rdst_y = OFFSET_PTR_2NX8U(pdst_y, 0, 0, ix);
    rdst_u = OFFSET_PTR_NX8U(pdst_u, 0, 0, (ix >> 1));
    rdst_v = OFFSET_PTR_NX8U(pdst_v, 0, 0, (ix >> 1));
    ...
    for (int iy = 0; iy < height; iy++) {
      ...
      xb_vec2Nx24 vec_y0 = IVP_MULUSI2NX8X16(vec_b0, Q15_0_0722, Q15_0_0722);
      IVP_MULUSA2N8XR16(vec_y0, vec_g0, Q15_0_7152);
      IVP_MULUSA2N8XR16(vec_y0, vec_r0, Q15_0_2126);

      IVP_SAV2NX8U_XP(IVP_PACKVRU2NX24(vec_y0, 15), a_store, rdst_y, WX);
      IVP_SAV2NX8UPOS_FP(a_store, rdst_y);
      rdst_y = OFFSET_PTR_2NX8U(rdst_y, 1, shift_y, 0);
    }
    ...
    for (int iy = 0; iy < height; iy += 2) {
      ...
      xb_vecNx16U vec_b0_16 = IVP_MOVNX16_FROM2NX8(vec_b0);
      xb_vecNx16U vec_g0_16 = IVP_MOVNX16_FROM2NX8(vec_g0);
      xb_vecNx16U vec_r0_16 = IVP_MOVNX16_FROM2NX8(vec_r0);

      vec_b0_16 =
          IVP_ADDNX16(IVP_SRLINX16(vec_b0_16, 8), IVP_ANDNX16(vec_b0_16, 255));
      vec_g0_16 =
          IVP_ADDNX16(IVP_SRLINX16(vec_g0_16, 8), IVP_ANDNX16(vec_g0_16, 255));
      vec_r0_16 =
          IVP_ADDNX16(IVP_SRLINX16(vec_r0_16, 8), IVP_ANDNX16(vec_r0_16, 255));

      a_load = IVP_LA2NX8U_PP(rsrc_rgb);
      xb_vec2Nx8U vec3;
      IVP_LAV2NX8U_XP(vec3, a_load, rsrc_rgb, 3 * WX);
      xb_vec2Nx8U vec4;
      IVP_LAV2NX8U_XP(vec4, a_load, rsrc_rgb,
                      3 * WX - 2 * XCHAL_IVPN_SIMD_WIDTH);
      xb_vec2Nx8U vec5;
      ...
      xb_vecNx16U vec_b1_16 = IVP_MOVNX16_FROM2NX8(vec_b1);
      vec_b1_16 =
          IVP_ADDNX16(IVP_SRLINX16(vec_b1_16, 8), IVP_ANDNX16(vec_b1_16, 255));
      xb_vecNx16U vec_b = IVP_ADDNX16(vec_b0_16, vec_b1_16);
      xb_vecNx48 vec_u = IVP_MULUSNX16(vec_b, Q15_0_5000);
      xb_vecNx48 vec_v = IVP_MULUSNX16(vec_b, -Q15_0_0458);

      xb_vecNx16U vec_r1_16 = IVP_MOVNX16_FROM2NX8(vec_r1);
      vec_r1_16 =
          IVP_ADDNX16(IVP_SRLINX16(vec_r1_16, 8), IVP_ANDNX16(vec_r1_16, 255));
      xb_vecNx16U vec_r = IVP_ADDNX16(vec_r0_16, vec_r1_16);
      IVP_MULUSANX16(vec_u, vec_r, -Q15_0_1146);
      IVP_MULUSANX16(vec_v, vec_r, Q15_0_5000);

      xb_vecNx16U vec_g1_16 = IVP_MOVNX16_FROM2NX8(vec_g1);
      vec_g1_16 =
          IVP_ADDNX16(IVP_SRLINX16(vec_g1_16, 8), IVP_ANDNX16(vec_g1_16, 255));
      xb_vecNx16U vec_g = IVP_ADDNX16(vec_g0_16, vec_g1_16);
      IVP_MULUSANX16(vec_u, vec_g, -Q15_0_3854);
      IVP_MULUSANX16(vec_v, vec_g, -Q15_0_4542);

      IVP_SAVNX8U_XP(IVP_ADDNX16(IVP_PACKVRNX48(vec_u, 17), 128), a_store,
                     rdst_u, WX / 2);
      IVP_SAVNX8UPOS_FP(a_store, rdst_u);
      rdst_u = OFFSET_PTR_NX8U(rdst_u, 1, shift_u, 0);

      IVP_SAVNX8U_XP(IVP_ADDNX16(IVP_PACKVRNX48(vec_v, 17), 128), a_store,
                     rdst_v, WX / 2);
      IVP_SAVNX8UPOS_FP(a_store, rdst_v);
      rdst_v = OFFSET_PTR_NX8U(rdst_v, 1, shift_v, 0);
    }
