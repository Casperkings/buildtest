#include "xi_lib.h"
{
  extern XI_ERR_TYPE xiTranspose_I16(const xi_pTile src, xi_pArray dst);
  ...
  src_t.pData =
      (__local uchar *)((local_src) + local_src_pitch * IMAGE_PAD + IMAGE_PAD);
  ...
  dst_t.pData = (__local uchar *)(local_dst);
  // XI_TILE_SET_BUFF_PTR    ((&dst_t), local_dst);
  // XI_TILE_SET_DATA_PTR    ((&dst_t), local_dst);
  ...
  err = xiTranspose_I16(&src_t, (xi_pArray)&dst_t);
}
