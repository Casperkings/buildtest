...
void launch_ocl_HarrisCornerU8(uint8_t *Src, uint32_t SrcPitch, uint8_t *Dst,
  ...
  const char *progSource = loadKernel("HarrisCornerU8.cl");
  cl_program program =
...
void run_reference(uint8_t *Src, uint32_t SrcPitch, uint8_t *Dst,
  ...
  cornerHarris5x5_U8_ref(((uint8_t *)Src) + SrcPitch * IMAGE_PAD + IMAGE_PAD,
                         Dst, width, height, SrcPitch, DstPitch, 5);
}
...
int main() {
  ...
  launch_ocl_HarrisCornerU8(gsrc, srcStride, gdst, dstWidth, srcWidth,
