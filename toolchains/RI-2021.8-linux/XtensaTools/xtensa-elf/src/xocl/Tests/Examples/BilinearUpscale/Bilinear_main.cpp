...
#define H_EXT 2
#define IMAGE_PAD 2
#define OUT_IMAGE_W (256 * 3)
#define OUT_IMAGE_H (280 * 3)
#define OUT_TILE_W (64 * 3)
...
void launch_ocl_kernel(uint8_t *Src, uint32_t SrcPitch, uint8_t *Dst,
  ...
  int xscale = (width << 18) / dstWidth;
  int yscale = (height << 18) / dstHeight;
  ...
  const char *progSource = loadKernel("Bilinear.cl");
  cl_program program =
  ...
  local_dst_size = ((OUT_TILE_W) * (OUT_TILE_H) * sizeof(cl_uchar));
  ...
  CHECK(clSetKernelArg(kernel, 8, sizeof(cl_int), (void *)&xscale));
  CHECK(clSetKernelArg(kernel, 9, sizeof(cl_int), (void *)&yscale));
  CHECK(clSetKernelArg(kernel, 10, sizeof(cl_int), (void *)&xshift));
  CHECK(clSetKernelArg(kernel, 11, sizeof(cl_int), (void *)&yshift));
  CHECK(clSetKernelArg(kernel, 12, local_src_size, (void *)NULL));
