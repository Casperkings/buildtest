...
void launch_ocl_kernel(uint8_t *SrcY, uint8_t *SrcUV, uint32_t SrcPitch,
  ...
  cl_mem clSrcUV = clCreateBuffer(
      context, CL_MEM_READ_ONLY,
      (sizeof(uint8_t) * (SrcPitch * (height / 2 + IMAGE_PAD + IMAGE_PAD))),
      NULL, &status);
  CHECK_ERROR(status);
  ...
  cl_mem clDstUV = clCreateBuffer(context, CL_MEM_WRITE_ONLY,
                                  (sizeof(uint8_t) * DstPitch * dstHeight / 2),
                                  NULL, &status);
  CHECK_ERROR(status);
  ...
  const char *progSource = loadKernel("LDC.cl");
  cl_program program =
  ...
  int local_srcY_size, local_srcUV_size;
  int local_dstY_size, local_dstUV_size;
  ...
  local_srcUV_size = local_srcY_size / 2;
  local_dstUV_size = local_dstY_size / 2;
  ...
  error = clEnqueueWriteBuffer(
      queue, clSrcUV, CL_TRUE, 0,
      (sizeof(uint8_t) * (SrcPitch * (height + IMAGE_PAD + IMAGE_PAD) / 2)),
      SrcUV, 0, NULL, NULL);
  CHECK_ERROR(error);

  error = clEnqueueWriteBuffer(queue, clInputX, CL_TRUE, 0,
                               (sizeof(uint16_t) * (height / OUTPUT_TILE_H) *
                                (width / OUTPUT_TILE_W) * 4),
                               InputX, 0, NULL, NULL);
  CHECK_ERROR(error);

  error = clEnqueueWriteBuffer(queue, clInputY, CL_TRUE, 0,
                               (sizeof(uint16_t) * (height / OUTPUT_TILE_H) *
                                (width / OUTPUT_TILE_W) * 4),
                               InputY, 0, NULL, NULL);
  CHECK_ERROR(error);
  ...
  CHECK(clSetKernelArg(kernel, 1, sizeof(cl_mem), (void *)&clSrcUV));
  CHECK(clSetKernelArg(kernel, 2, sizeof(cl_int), (void *)&SrcPitch));
  CHECK(clSetKernelArg(kernel, 3, sizeof(cl_mem), (void *)&clDstY));
  CHECK(clSetKernelArg(kernel, 4, sizeof(cl_mem), (void *)&clDstUV));
  ...
  CHECK(clSetKernelArg(kernel, 10, sizeof(cl_int), (void *)&OutputTileW));
  CHECK(clSetKernelArg(kernel, 11, sizeof(cl_int), (void *)&OutputTileH));
  CHECK(clSetKernelArg(kernel, 12, sizeof(cl_mem), (void *)&clInputX));
  CHECK(clSetKernelArg(kernel, 13, sizeof(cl_mem), (void *)&clInputY));
  CHECK(clSetKernelArg(kernel, 14, sizeof(cl_mem), (void *)&clInputBB));
  CHECK(clSetKernelArg(kernel, 15, local_srcY_size, (void *)NULL));
  CHECK(clSetKernelArg(kernel, 16, local_srcUV_size, (void *)NULL));
  CHECK(clSetKernelArg(kernel, 17, local_dstY_size, (void *)NULL));
  CHECK(clSetKernelArg(kernel, 18, local_dstUV_size, (void *)NULL));
  CHECK(clSetKernelArg(kernel, 19, sizeof(cl_mem), (void *)&clErrCodes));
  ...
  CHECK(clEnqueueReadBuffer(queue, clDstY, CL_TRUE, 0,
                            ...
                            NULL, NULL));
  CHECK(clEnqueueReadBuffer(queue, clDstUV, CL_TRUE, 0,
                            (sizeof(cl_char) * DstPitch * dstHeight / 2), DstUV,
                            0, NULL, NULL));
...
int main() {
  ...
  gsrcY = (uint8_t *)malloc(srcStride * (srcHeight + IMAGE_PAD + IMAGE_PAD) *
                            srcBytes);
  gsrcUV = (uint8_t *)malloc(
      (srcStride * (srcHeight + IMAGE_PAD + IMAGE_PAD) * srcBytes) / 2);
  ...
  launch_ocl_kernel(gsrcY, gsrcUV, srcStride, gdstY, gdstUV, dstWidth, srcWidth,
                    srcHeight, dstWidth, dstHeight, InputX, InputY, InputBB);
