#include "xi_lib.h"
{
  extern XI_ERR_TYPE xiCvtColor_U8_RGB2YUV_420_BT709(
  ...
  XI_TILE_SET_BUFF_SIZE((&dstU_t), (local_dst_pitch * (outTileH)) / 4);
  XI_TILE_SET_WIDTH((&dstU_t), outTileW / 2);
  XI_TILE_SET_HEIGHT((&dstU_t), outTileH / 2);
  XI_TILE_SET_PITCH((&dstU_t), (local_dst_pitch) / 2);
  ...
  XI_TILE_SET_BUFF_SIZE((&dstV_t), (local_dst_pitch * (outTileH)) / 4);
  XI_TILE_SET_WIDTH((&dstV_t), outTileW / 2);
  XI_TILE_SET_HEIGHT((&dstV_t), outTileH / 2);
  XI_TILE_SET_PITCH((&dstV_t), (local_dst_pitch) / 2);
  ...
  int err;
  err = xiCvtColor_U8_RGB2YUV_420_BT709((xi_pArray)&src_t, (xi_pArray)&dstY_t,
