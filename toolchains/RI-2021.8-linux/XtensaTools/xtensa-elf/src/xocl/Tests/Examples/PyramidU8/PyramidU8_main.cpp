...
void launch_ocl_Pyramid(uint8_t *Src, uint32_t SrcPitch, uint8_t *Dst,
  ...
  const char *progSource = loadKernel("PyramidU8.cl");
  cl_program program =
  ...
  local_dst_size = ((TILE_W>>1) * (TILE_H>>1) * sizeof(cl_uchar));
...
int main() {
  ...
  launch_ocl_Pyramid(gsrc, srcStride, gdst, dstWidth, srcWidth, srcHeight,
                      dstWidth, dstHeight);
