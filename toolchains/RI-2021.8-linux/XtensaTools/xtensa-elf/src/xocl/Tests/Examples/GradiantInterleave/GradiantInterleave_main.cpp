...
void launch_ocl_GradiantInterleave(uint8_t *Src, uint32_t SrcPitch,
                                   int16_t *DstXY, const uint32_t DstXYPitch,
  ...
  cl_mem clDstXY =
      clCreateBuffer(context, CL_MEM_WRITE_ONLY,
                     (sizeof(int16_t) * DstXYPitch * dstHeight), NULL, &status);
  CHECK_ERROR(status);

  cl_event event = clCreateUserEvent(context, &status);
  CHECK_ERROR(status);

  const char *progSource = loadKernel("GradiantInterleave.cl");
  cl_program program =
  ...
  std::cout << SrcPitch << "  " << DstXYPitch << "  " << width << "  " << height
  ...
  local_dst_size = ((TILE_W * 2) * (TILE_H) * sizeof(cl_short));

  std::cout << " SrcPitch = " << SrcPitch << "\n";
  std::cout << " DstXYPitch = " << DstXYPitch << "\n";
  ...
  CHECK(clSetKernelArg(kernel, 2, sizeof(cl_mem), (void *)&clDstXY));
  CHECK(clSetKernelArg(kernel, 3, sizeof(cl_int), (void *)&DstXYPitch));
  ...
  CHECK(clEnqueueReadBuffer(queue, clDstXY, CL_TRUE, 0,
                            (sizeof(cl_short) * DstXYPitch * dstHeight), DstXY,
                            0, NULL, NULL));
...
int main() {
  ...
  gsrc = (uint8_t *)malloc(srcStride * (srcHeight + IMAGE_PAD + IMAGE_PAD) *
                           srcBytes);
  gdstXY = (int16_t *)malloc(dstWidth * dstHeight * dstBytes);
  gdstXY_ref = (int16_t *)malloc(dstWidth * dstHeight * dstBytes);
  if (!gsrc || !gdstXY || !gdstXY_ref) {
  ...
  launch_ocl_GradiantInterleave(gsrc, srcStride, gdstXY, dstWidth, srcWidth,
                                srcHeight, dstWidth, dstHeight);

  run_reference(gsrc, srcStride, gdstXY_ref, dstWidth, srcWidth, srcHeight,
                dstWidth, dstHeight);
  ...
  fwrite(gdstXY, dstWidth * dstHeight * 2, 1, fpOut);
  fclose(fpOut);
  ...
  fwrite(gdstXY_ref, dstWidth * dstHeight * 2, 1, fpOut);
  fclose(fpOut);

  printf("Compare XY: ");

  if (memcmp(gdstXY_ref, gdstXY, dstWidth * dstHeight * 2) == 0) {
    printf("PASS \n");
  } else {
    printf("FAIL \n");
  }
