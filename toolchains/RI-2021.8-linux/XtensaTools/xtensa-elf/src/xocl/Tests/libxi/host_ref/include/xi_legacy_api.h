...
#define XI_RESIZE_BILINEAR_U8_TMP_CAPACITY(dst_width, dst_height,              \
                                           src_full_width, src_full_height)    \
  32
#define XI_RESIZE_BILINEAR_U8_TMP_SIZE(dst_width, dst_height, src_full_width,  \
                                       src_full_height)                        \
  (xi_size) { 32, 1 }
#define XI_RESIZE_BILINEAR_U8_DST_CAPACITY(dst_width, dst_height,              \
                                           src_full_width, src_full_height)    \
  ((dst_width) * (dst_height))
#define XI_RESIZE_BILINEAR_U8_DST_SIZE(dst_width, dst_height, src_full_width,  \
                                       src_full_height)                        \
  (xi_size) { (dst_width), (dst_height) }

#define XI_RESIZE_BILINEAR_U8Oa_TMP_CAPACITY(dst_width, dst_height,            \
                                             src_full_width, src_full_height)  \
  32
#define XI_RESIZE_BILINEAR_U8Oa_TMP_SIZE(dst_width, dst_height,                \
                                         src_full_width, src_full_height)      \
  (xi_size) { 32, 1 }
#define XI_RESIZE_BILINEAR_U8Oa_DST_CAPACITY(dst_width, dst_height,            \
                                             src_full_width, src_full_height)  \
  (XI_ALIGN_VALN(dst_width) * (dst_height))
#define XI_RESIZE_BILINEAR_U8Oa_DST_SIZE(dst_width, dst_height,                \
                                         src_full_width, src_full_height)      \
  (xi_size) { (dst_width), (dst_height) }

#define XI_RESIZE_BILINEAR_S16_TMP_CAPACITY(dst_width, dst_height,             \
                                            src_full_width, src_full_height)   \
  64
#define XI_RESIZE_BILINEAR_S16_TMP_SIZE(dst_width, dst_height, src_full_width, \
                                        src_full_height)                       \
  (xi_size) { 32, 1 }
#define XI_RESIZE_BILINEAR_S16_DST_CAPACITY(dst_width, dst_height,             \
                                            src_full_width, src_full_height)   \
  2 * ((dst_width) * (dst_height))
#define XI_RESIZE_BILINEAR_S16_DST_SIZE(dst_width, dst_height, src_full_width, \
                                        src_full_height)                       \
  (xi_size) { (dst_width), (dst_height) }

#define XI_RESIZE_BILINEAR_S16Oa_TMP_CAPACITY(dst_width, dst_height,           \
                                              src_full_width, src_full_height) \
  64
#define XI_RESIZE_BILINEAR_S16Oa_TMP_SIZE(dst_width, dst_height,               \
                                          src_full_width, src_full_height)     \
  (xi_size) { 32, 1 }
#define XI_RESIZE_BILINEAR_S16Oa_DST_CAPACITY(dst_width, dst_height,           \
                                              src_full_width, src_full_height) \
  2 * (XI_ALIGN_VALN(dst_width) * (dst_height))
#define XI_RESIZE_BILINEAR_S16Oa_DST_SIZE(dst_width, dst_height,               \
                                          src_full_width, src_full_height)     \
  (xi_size) { (dst_width), (dst_height) }
...
#define XI_RESIZE_AREA_U8Oa_TMP_CAPACITY(dst_height, src_full_width)           \
  (size_t) {                                                                   \
    2 * (XI_ALIGN_VALN(src_full_width)) * XI_ALIGN_VALN(dst_height) +          \
        (2 * XI_SIMD_WIDTH - 1)                                                \
  }

/* bilinear resize */
#define xiResizeBilinear_U8_Q13_18(src, dst, tmp, ...)                         \
  xiResizeBilinear2_U8_Q13_18(src, dst, __VA_ARGS__)
#define xiResizeBilinear_U8Oa_Q13_18 xiResizeBilinear_U8_Q13_18

#define xiResizeBilinear_S16_Q13_18(src, dst, tmp, ...)                        \
  xiResizeBilinear2_S16_Q13_18(src, dst, __VA_ARGS__)
#define xiResizeBilinear_S16Oa_Q13_18(src, dst, tmp, ...)                      \
  xiResizeBilinear2_S16Oa_Q13_18(src, dst, __VA_ARGS__)
...
_XI_API_ XI_ERR_TYPE xiReduceAffineScale(const xi_affine *input,
                                         xi_affine *scaled, float *scale_factor,
                                         float max_scale XI_DEFAULT(2.0f));
_XI_API_ XI_ERR_TYPE xiPrepareAffine(const xi_affine *affine, xi_size dst_size,
                                     int pixel_res, xi_bool aligned,
                                     xi_separable_affine *decomposed,
                                     unsigned *tmp_buf_size,
                                     unsigned *dst_buf_size);
_XI_API_ XI_ERR_TYPE xiProjectRectAffine(
    const xi_separable_affine *inverse_affine, xi_point pos, xi_size size,
    xi_point *projected_pos, xi_size *projected_size);

_XI_API_ XI_ERR_TYPE
xiWarpAffine_U8_Q13_18(const xi_pTile src, xi_pTile dst, xi_pArray tmpbuf,
                       const xi_separable_affine *decomposed);
_XI_API_ XI_ERR_TYPE
xiWarpAffine_U8A_Q13_18(const xi_pTile src, xi_pTile dst, xi_pArray tmpbuf,
                        const xi_separable_affine *decomposed);
_XI_API_ XI_ERR_TYPE
xiWarpAffine_S16_Q13_18(const xi_pTile src, xi_pTile dst, xi_pArray tmpbuf,
                        const xi_separable_affine *decomposed);
_XI_API_ XI_ERR_TYPE
xiWarpAffine_S16A_Q13_18(const xi_pTile src, xi_pTile dst, xi_pArray tmpbuf,
                         const xi_separable_affine *decomposed);

/* BRIEF descriptor */
#define XI_BRIEF_PATTERN_SIZE 5375
_XI_API_ XI_ERR_TYPE xiBRIEF_48x48_InitializePattern(xi_pArray pattern);
#define xiBRIEF16_48x48_S16U8(src, pattern, ...)                               \
  xiBRIEF16_48x48_S16U8_2(src, __VA_ARGS__)
#define xiBRIEF16_48x48_U16U8(src, pattern, ...)                               \
  xiBRIEF16_48x48_U16U8_2(src, __VA_ARGS__)
#define xiBRIEF32_48x48_S16U8(src, pattern, ...)                               \
  xiBRIEF32_48x48_S16U8_2(src, __VA_ARGS__)
#define xiBRIEF32_48x48_U16U8(src, pattern, ...)                               \
  xiBRIEF32_48x48_U16U8_2(src, __VA_ARGS__)
#define xiBRIEF64_48x48_S16U8(src, pattern, ...)                               \
  xiBRIEF64_48x48_S16U8_2(src, __VA_ARGS__)
#define xiBRIEF64_48x48_U16U8(src, pattern, ...)                               \
  xiBRIEF64_48x48_U16U8_2(src, __VA_ARGS__)
...
#define XI_MSER_PARAMS_INIT                                                    \
  {                                                                            \
    5, 60, 14400, (XI_Q15)((0.25) * (1 << 15) + 0.5),                          \
        (XI_Q15)((0.20) * (1 << 15) + 0.5)                                     \
  }

_XI_API_ XI_ERR_TYPE xiMSER_U8(const xi_pArray src, xi_pArray dst,
                               xi_pArray tmp, const xi_mser_params *params);

/* Haar */
_XI_API_ XI_ERR_TYPE xiHaarStump_U16U8_xstep1(const xi_pArray integral_image,
                                              const xi_pArray stdDev,
                                              int16_t *cascade,
                                              xi_pArray detect_map, int ystep,
                                              xi_size window, int numStages,
                                              int *pskipSum);
_XI_API_ XI_ERR_TYPE xiHaarStump_U16U8_xstep2(const xi_pArray integral_image,
                                              const xi_pArray stdDev,
                                              int16_t *cascade,
                                              xi_pArray detect_map, int ystep,
                                              xi_size window, int numStages,
...
/* HOG Histogram */
_XI_API_ XI_ERR_TYPE xiHOGWeighted_S16U32_Q16_15(const xi_pTile src_mag,
