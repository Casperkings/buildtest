...
// for handling the DMA transfers. In this case the xi_tile and xi_frame
// structures must be expaded to reflect the extra structre elements added to
// xvFrame and xvTile structures.
...
#define XI_TYPE_CHANNELS_MASK                                                  \
  (((1 << XI_TYPE_CHANNELS_BITS) - 1) << XI_TYPE_ELEMENT_SIZE_BITS)

#define XI_MAKETYPE(flags, depth, channels)                                    \
  (((depth) * (channels)) | (((channels)-1) << XI_TYPE_ELEMENT_SIZE_BITS) |    \
   (flags))
#define XI_CUSTOMTYPE(type) XI_MAKETYPE(0, sizeof(type), 1)

#define XI_TYPE_ELEMENT_SIZE(type) ((type) & (XI_TYPE_ELEMENT_SIZE_MASK))
#define XI_TYPE_ELEMENT_TYPE(type)                                             \
  ((type) &                                                                    \
   (XI_TYPE_SIGNED_BIT | XI_TYPE_CHANNELS_MASK | XI_TYPE_ELEMENT_SIZE_MASK))
#define XI_TYPE_IS_TILE(type)   ((type) & (XI_TYPE_TILE_BIT))
#define XI_TYPE_IS_SIGNED(type) ((type) & (XI_TYPE_SIGNED_BIT))
#define XI_TYPE_CHANNELS(type)                                                 \
  ((((type) & (XI_TYPE_CHANNELS_MASK)) >> (XI_TYPE_ELEMENT_SIZE_BITS)) + 1)

// XV_MAKETYPE accepts 3 parameters
// 1: Denotes whether the entity is a tile (XV_TYPE_TILE_BIT is set) or an array
// (XV_TYPE_TILE_BIT is unset),
//    and also if the data is a signed or unsigned.
// 2: Denotes number of bytes per pel.
//    1 implies the data is 8bit, 2 implies the data is 16bit and 4 implies the
//    data is 32bit.
...
#define XI_FLOAT XI_MAKETYPE(0, 4, 1)
...
#define XI_FRAME_SET_BUFF_PTR(pFrame, pBuff)                                   \
  (pFrame)->pFrameBuff = ((void *)(pBuff))

#define XI_FRAME_GET_BUFF_SIZE(pFrame) ((pFrame)->frameBuffSize)
#define XI_FRAME_SET_BUFF_SIZE(pFrame, buffSize)                               \
  (pFrame)->frameBuffSize = ((uint32_t)(buffSize))

#define XI_FRAME_GET_DATA_PTR(pFrame) ((pFrame)->pFrameData)
#define XI_FRAME_SET_DATA_PTR(pFrame, pData)                                   \
  (pFrame)->pFrameData = ((void *)(pData))

#define XI_FRAME_GET_WIDTH(pFrame) ((pFrame)->frameWidth)
#define XI_FRAME_SET_WIDTH(pFrame, width)                                      \
  (pFrame)->frameWidth = ((int32_t)(width))

#define XI_FRAME_GET_HEIGHT(pFrame) ((pFrame)->frameHeight)
#define XI_FRAME_SET_HEIGHT(pFrame, height)                                    \
  (pFrame)->frameHeight = ((int32_t)(height))

#define XI_FRAME_GET_PITCH(pFrame) ((pFrame)->framePitch)
#define XI_FRAME_SET_PITCH(pFrame, pitch)                                      \
  (pFrame)->framePitch = ((int32_t)(pitch))

#define XI_FRAME_GET_PIXEL_RES(pFrame) ((pFrame)->pixelRes)
#define XI_FRAME_SET_PIXEL_RES(pFrame, pixRes)                                 \
  (pFrame)->pixelRes = ((uint8_t)(pixRes))

#define XI_FRAME_GET_PIXEL_FORMAT(pFrame) ((pFrame)->pixelPackFormat)
#define XI_FRAME_SET_PIXEL_FORMAT(pFrame, pixelFormat)                         \
  (pFrame)->pixelPackFormat = ((uint8_t)(pixelFormat))
...
#define XI_ARRAY_SET_BUFF_PTR(pArray, pBuff)                                   \
  (pArray)->pBuffer = ((void *)(pBuff))

#define XI_ARRAY_GET_BUFF_SIZE(pArray) ((pArray)->bufferSize)
#define XI_ARRAY_SET_BUFF_SIZE(pArray, buffSize)                               \
  (pArray)->bufferSize = ((uint32_t)(buffSize))

#define XI_ARRAY_GET_DATA_PTR(pArray) ((pArray)->pData)
#define XI_ARRAY_SET_DATA_PTR(pArray, pArrayData)                              \
  (pArray)->pData = ((void *)(pArrayData))
...
#define XI_ARRAY_SET_HEIGHT(pArray, value)                                     \
  (pArray)->height = ((uint16_t)(value))

#define XI_ARRAY_GET_STATUS_FLAGS(pArray) ((pArray)->status)
#define XI_ARRAY_SET_STATUS_FLAGS(pArray, value)                               \
  (pArray)->status = ((uint8_t)(value))
...
#define XI_ARRAY_SET_CAPACITY(pArray, value)                                   \
  (pArray)->pitch = ((int32_t)(value))

#define XI_ARRAY_GET_ELEMENT_TYPE(pArray)                                      \
  ((XI_ARRAY_GET_TYPE(pArray)) &                                               \
   (XI_TYPE_SIGNED_BIT | XI_TYPE_CHANNELS_MASK | XI_TYPE_ELEMENT_SIZE_MASK))
#define XI_ARRAY_GET_ELEMENT_SIZE(pArray)                                      \
  ((XI_ARRAY_GET_TYPE(pArray)) & (XI_TYPE_ELEMENT_SIZE_MASK))
#define XI_ARRAY_IS_TILE(pArray)                                               \
  ((XI_ARRAY_GET_TYPE(pArray)) & (XI_TYPE_TILE_BIT))

#define XI_ARRAY_GET_AREA(pArray)                                              \
  (((pArray)->width) * ((int32_t)(pArray)->height))
...
#define XI_TILE_SET_FRAME_PTR(pTile, ptrFrame)                                 \
  (pTile)->pFrame = ((xi_frame *)(ptrFrame))
...
#ifdef XI_XV_TILE_COMPATIBILITY
#define XI_TILE_SET_EDGE_WIDTH(pTile, eWidth)                                  \
  XV_TILE_SET_EDGE_WIDTH((xvpTile)pTile, eWidth)
#else
#define XI_TILE_SET_EDGE_WIDTH(pTile, eWidth)                                  \
  ((pTile)->edgeWidth = (uint16_t)eWidth)
#endif

#define XI_TILE_GET_EDGE_HEIGHT(pTile) ((pTile)->edgeHeight)
#ifdef XI_XV_TILE_COMPATIBILITY
#define XI_TILE_SET_EDGE_HEIGHT(pTile, eHeight)                                \
  XV_TILE_SET_EDGE_HEIGHT((xvpTile)pTile, eHeight)
#else
#define XI_TILE_SET_EDGE_HEIGHT(pTile, eHeight)                                \
  ((pTile)->edgeHeight = (uint16_t)eHeight)
#endif
...
#define SETUP_TILE(pTile, pBuf, bufSize, pFrame, width, height, pitch, type,   \
                   edgeWidth, edgeHeight, x, y, alignType)                     \
  {                                                                            \
    int32_t tileType, bytesPerPixel, channels, bytesPerPel;                    \
    uint8_t *edgePtr = (uint8_t *)pBuf, *dataPtr;                              \
    int alignment = 63;                                                        \
    if ((alignType == EDGE_ALIGNED_32) || (alignType == DATA_ALIGNED_32)) {    \
      alignment = 31;                                                          \
    }                                                                          \
    ...
    XI_TILE_SET_DATA_PTR(pTile,                                                \
                         edgePtr + ((edgeHeight) * (pitch)*bytesPerPel +       \
                                    (edgeWidth)*bytesPerPixel));               \
...
#define SETUP_FRAME(pFrame, pFrameBuffer, buffSize, width, height, pitch,      \
                    padWidth, padHeight, pixRes, numCh, paddingType,           \
                    paddingVal)                                                \
  {                                                                            \
    ...
    XI_FRAME_SET_DATA_PTR(                                                     \
        pFrame,                                                                \
        pFrameBuffer + (pitch * padHeight + padWidth * numCh) * pixRes);       \
