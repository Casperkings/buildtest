...
#define IMAGE_W       512
#define IMAGE_H       512
#define OUT_IMAGE_W   (IMAGE_H)      //(IMAGE_W/32)
#define OUT_IMAGE_H   (IMAGE_W / 32) //(IMAGE_H)
#define SRC_BIT_DEPTH 8
#define DST_BIT_DEPTH 32
#define TILE_W        128
#define TILE_H        64
#define OUT_TILE_W    (TILE_H)      //(TILE_W/32)
#define OUT_TILE_H    (TILE_W / 32) //(TILE_H)
...
XI_ERR_TYPE SumOfSquaredDiffsU8S32_Ref(const uint8_t *pSrc1,
                                       ...
                                       int32_t nlength);
void launch_ocl_SumOfSquaredDiffs(uint8_t *Src, uint8_t *Src1,
                                  uint32_t SrcPitch, uint8_t *Dst,
                                  const uint32_t DstPitch, int32_t width,
                                  int32_t height, int32_t dstWidth,
                                  int32_t dstHeight) {
  cl_int status;
  ...
  cl_mem clSrc1 = clCreateBuffer(context, CL_MEM_READ_ONLY,
                                 (sizeof(uint8_t) * (SrcPitch)), NULL, &status);
  CHECK_ERROR(status);
  ...
  const char *progSource = loadKernel("SumOfSquaredDiffsU8S32.cl");
  cl_program program =
  ...
  int local_src_size, local_src1_size;
  int local_dst_size;
  local_src_size = ((TILE_W + IMAGE_PAD + IMAGE_PAD) *
                    (TILE_H + IMAGE_PAD + IMAGE_PAD) * sizeof(cl_uchar));
  local_src1_size = TILE_W * sizeof(cl_uchar);
  local_dst_size = ((TILE_W / 32) * (TILE_H) * sizeof(cl_uint));
  ...
  error =
      clEnqueueWriteBuffer(queue, clSrc1, CL_TRUE, 0,
                           (sizeof(uint8_t) * (SrcPitch)), Src1, 0, NULL, NULL);
  CHECK_ERROR(error);

  CHECK(clSetKernelArg(kernel, 0, sizeof(cl_mem), (void *)&clSrc));
  CHECK(clSetKernelArg(kernel, 1, sizeof(cl_mem), (void *)&clSrc1));
  ...
  CHECK(clSetKernelArg(kernel, 10, local_src1_size, (void *)NULL));
  CHECK(clSetKernelArg(kernel, 11, local_dst_size, (void *)NULL));
  CHECK(clSetKernelArg(kernel, 12, sizeof(cl_mem), (void *)&clErrCodes));
...
void run_reference(uint8_t *Src, uint8_t *Src1, uint32_t SrcPitch, uint8_t *Dst,
  ...
  SumOfSquaredDiffsU8S32_Ref(Src1, Src, (int32_t *)Dst, width, height, 32);
}
...
int main() {
  ...
  launch_ocl_SumOfSquaredDiffs(gsrc, gsrc1, srcStride, gdst, dstWidth, srcWidth,
