...
void launch_ocl_kernel(uint8_t *Src, uint32_t SrcPitch, uint8_t *Dst,
  ...
  cl_int status;
#if NATIVE_KERNEL
  const int8_t filtCoeff[29] = {6,  17, 8,  13, 3, 2,  10, 19, 5, 1,
                                16, 3,  18, 15, 6, 13, 4,  16, 7, 15,
                                2,  15, 15, 20, 2, 15, 15, 20, 7};
#elif XT_OCL_EXT
  const int8_t filtCoeff[25] = {6, 17, 8,  13, 2, 3,  2,  10, 19, 15, 5,  1, 16,
                                3, 15, 18, 15, 6, 13, 20, 4,  16, 7,  15, 7};
#else
  const short filtCoeff[25] = {6, 17, 8,  13, 2, 3,  2,  10, 19, 15, 5,  1, 16,
                               3, 15, 18, 15, 6, 13, 20, 4,  16, 7,  15, 7};
#endif
  ...
  cl_mem clFiltercoef = clCreateBuffer(context, CL_MEM_READ_ONLY,
                                       (sizeof(short) * 25), NULL, &status);
  CHECK_ERROR(status);
  cl_event event = clCreateUserEvent(context, &status);
  CHECK_ERROR(status);

  const char *progSource = loadKernel("Filter2D.cl");
  cl_program program =
  ...
  error = clEnqueueWriteBuffer(queue, clFiltercoef, CL_TRUE, 0,
                               (sizeof(short) * 25), filtCoeff, 0, NULL, NULL);
  CHECK_ERROR(error);
  ...
  CHECK(clSetKernelArg(kernel, 11, sizeof(cl_mem), (void *)&clFiltercoef));
