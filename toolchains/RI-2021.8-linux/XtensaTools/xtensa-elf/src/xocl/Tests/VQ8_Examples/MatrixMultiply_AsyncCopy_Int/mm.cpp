...
void matrix_multiply_ocl() {
  ...
  CHECK_ERROR(
      clSetKernelArg(kernel, 6, (128 * wA * sizeof(cl_char)), (void *)NULL));
  CHECK_ERROR(
      clSetKernelArg(kernel, 7, (128 * hB * sizeof(cl_char)), (void *)NULL));
  CHECK_ERROR(
      clSetKernelArg(kernel, 8, (128 * 128 * sizeof(cl_int)), (void *)NULL));

  size_t local[2] = {l0, l1};
  size_t global[2] = {hC / 128, wC / 128};
