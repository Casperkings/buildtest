...
int filt_IVP_New(const xi_pTile src, xi_pTile dst, int8_t *filtCoeff_1) {
  ...
  for (x = 0; x < XI_TILE_GET_WIDTH(dst); x += 128) {
    ...
    for (y = 0; y < XI_TILE_GET_HEIGHT(dst); y++) {
      ...
      IVP_LA2NX8_XP(vec41, a, pvecImg, Pitch - 128);
      // vec41 = IVP_LV2NX8U_I(pvecImg, 128);
      ...
      IVP_SVNX8U_XP(vecL >> 8, pvecDst_1, 64);
      IVP_SVNX8U_XP(vecH >> 8, pvecDst_1, 64);
      pvecDst_1 += (DstPitch >> 6) - 2;
