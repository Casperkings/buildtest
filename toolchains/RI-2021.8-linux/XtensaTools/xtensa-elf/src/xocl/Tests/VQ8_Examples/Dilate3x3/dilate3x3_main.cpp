...
void launch_ocl_dilate(uint8_t *Src, uint32_t SrcPitch, uint8_t *Dst,
  ...
  const char *progSource = loadKernel("dilate.cl");
  cl_program program =
...
void run_reference(uint8_t *Src, uint32_t SrcPitch, uint8_t *Dst,
  ...
  xiDilate_3x3_U8_ref(&src_t, (xi_pArray)&dst_t);
}
...
int main() {
  ...
  launch_ocl_dilate(gsrc, srcStride, gdst, dstWidth, srcWidth, srcHeight,
                      dstWidth, dstHeight);
