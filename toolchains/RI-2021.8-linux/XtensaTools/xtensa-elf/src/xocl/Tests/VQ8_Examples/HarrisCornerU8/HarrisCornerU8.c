...
void xvCornerHarris5x5Tile(int8_t *p_s8Dx, int8_t *p_s8Dy, uint8_t *p_u8In,
  ...
  uint32_t loop = ((u32Height + 4) * s32SrcStride + 127) >> 7;
  ...
  for (j = 0; j < u32Width /* && totalCount < u32nCornersMax*/; j += 128) {
    i = 0;
    ...
    for (k = 0; k < 5; k++) {
      ...
      IVP_LA2NX8_XP(dvecSx1, vaX, pdvecSx, (s32SrcStride - 128));

      IVP_LA2NX8_IP(dvecSy0, vaY, pdvecSy);
      IVP_LA2NX8_XP(dvecSy1, vaY, pdvecSy, (s32SrcStride - 128));
...
#define ROUND_FACTOR_ORDER2 10
    ...
    for (i = 1; i < u32Height; i++) {
      ...
      IVP_LA2NX8_XP(dvecSx1, vaXTemp, pdvecSxTemp, (s32SrcStride - 128));

      IVP_LA2NX8_IP(dvecSy0, vaYTemp, pdvecSyTemp);
      IVP_LA2NX8_XP(dvecSy1, vaYTemp, pdvecSyTemp, (s32SrcStride - 128));
