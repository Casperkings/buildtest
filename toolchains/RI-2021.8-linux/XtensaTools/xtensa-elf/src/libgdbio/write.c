...
int _write_r(struct _reent * reent, int file, const void * buf, size_t count)
{
    ...
    case LIBIO_SIM:
        {
            ...
            register const void *a4 __asm__ ("a4") = buf;
            register int a5 __asm__ ("a5") = count;
    ...
    case LIBIO_GDB:
        {
            ...
            if ((file = _gdbio_map_fd(reent, file)) < 0)
                return -1;
            ret = _gdbio_syscall(GDBIO_TARGET_SYSCALL_WRITE, buf, count, 0, file);
