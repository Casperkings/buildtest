...
class SYSTEMC_API sc_port_base
: public sc_object
{
...
#ifdef _MSC_VER
template class SYSTEMC_API std::allocator<sc_port_base*>;
template class SYSTEMC_API std::vector   <sc_port_base*>;
#endif
...
class SYSTEMC_API sc_port_registry
{
...
extern SYSTEMC_API void sc_warn_port_constructor();
...
sc_port_b<IF>::add_interface( sc_interface* interface_ )
{
    ...
    int size = (int)m_interface_vec.size();
...
{
	return (int)m_interface_vec.size();
}
                  ...
                  sc_event_finder* event_finder_ ) const
{
    if ( m_bind_info == 0 )
    {
        int if_n = (int)m_interface_vec.size();
